...
is the reference class, so its coefficients are fixed to zero. The
fitted object retains its native class order. Use
`latInspect(fit, sort = TRUE)` for sorted output without changing the
reference class or the fitted object.
