...
class latentloglik_combination_softmax: public estimators {
...
public:
  ...
  void G() {
    ...
    arma::vec vector = classes;
    arma::mat jacobian = -vector * vector.t();
    jacobian.diag() = vector % (1-vector);
    arma::vec dale = jacobian.t() * gclasses;
    gclasses(indices_target_classes) = dale(indices_target_classes);
