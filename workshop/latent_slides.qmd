...
- [Categorical indicators example](#categorical-indicators-example)
- [Model Fit Indices](#model-fit-indices)
- [Classification Diagnostics](#classification-diagnostics)
- [Confidence intervals](#confidence-intervals)
- [Continuous indicators example](#continuous-indicators-example)
- [Mixed indicators example](#mixed-indicators-example)
- [Two-step LCA analysis with covariates](#two-step-lca-analysis-with-covariates)
- [Factor Analysis](#factor_analysis)
- [Factor Analysis (multigroup)](#factor-analysis-multigroup)
- [Positive-definite constraints](#positive-definite-constraints)
- [Positive-definite latent covariances](#positive-definite-latent-covariances)
...
![](figures/surprised2.jpg){width=400px style="vertical-align:middle; margin-right:0.5em;"}

:::
::: {.column width="40%"}
![](figures/fast.jpg){width=500px style="vertical-align:middle; margin-right:0.5em;"}
:::
:::
...
```{r eval=TRUE, echo=TRUE, messages=FALSE}
# Covariates:
X <- empathy[, 7:8]
names(X)
...
# Structural model:
fit <- lca(data = empathy[, 1:6], X = X, model = fit0,
           item = rep("gaussian", ncol(empathy[, 1:6])),
           nclasses = 4L)
fit@loglik                # -1806.426
fit@penalized_loglik      # -1809.614
fit@Optim$opt$iterations  # 38
fit@Optim$opt$convergence # TRUE
