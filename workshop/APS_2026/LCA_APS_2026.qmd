...
- Is a direct translation of linear models.
- Implies that everything that is not True score becomes random error.
  - Age in years carries error by months, for example.
...
- Requires us to think and expand on what we are measuring, how is it measured, and what specific elements can affect it?
- Specific variance can be interpret as “systematic” error/bias.

# Modeling framework

- How can we approach this partition?
- Need to use multiple indicators/items to estimate the True underlying construct.
...
- Example, Math ability
  ...
  - All are examples of math ability, but at different levels, your indicators have to be appropriate for the level of interest and/or the sample.

# “There is no true model…”

- Models are parsimonious approximations of real world phenomena.

- Equivalent models may just be equally appropriate.
...
- Person-centered approaches describe similarities and differences among individuals with respect to how variables relate to each other and are predicated on the assumption that the population is heterogeneous with respect to the relationships between variables.
...
- Ruscio and Ruscio (2008) outline the differences between the two:
  ...
  - **Continuous latent variables (Factor Analysis)**: “…people or objects differ quantitatively along one or more continua.”
...
- Linear relation between the latent variable and the indicators through a link function (e.g, logistic function for categorical indicators).
...
- Suppose that there are
  ...
  - $r_j = 1,...,R_j$ response categories. The contingency table formed by cross-tabulating the $J$ variables has $W = \prod^J_{j=1} R_j$ cells.
...
- In Latent Variable Modeling we need to define the constructs that we are measuring.
- Related to construct validity: what do the items represent or reflect?
- with the previous items, we may be measuring **Political Activism**.
...
- **Akaike information criterion** (AIC; Akaike, 1987): approximated measure of the out-of sample prediction accuracy.
...
- The BIC generally penalizes free parameters more strongly than the AIC, though it also depends on the sample size $n$.
...
- Estimate many LCAs with increasing numbers of classes $C$.
- Compare the models ICs.
- Choose 2 or 3 models that present *similar* fit to evaluate their interpretability.
...
- We will continue with the 3 class solution.

- From the chosen solution with 3 classes, we need to **assign names to each latent class**.

  - based on what characterizes them (qualitative differences among the groups).
...
During the last 12 months, have you done any of the following? Have you... (Yes =1, No=2)
...
- What names would you choose for the classes?
  - It seems that we have a *Not politically active* class, comprising 59% of the sample.
  - Then we have a class of *Somewhat active* people, comprising 35% of the sample.
  - And lastly we have a *Very active* class, comprising 6% of the sample.
...
- With the class membership proportions based on modal assignment
...
- **AvePP**: average predicted probability for the subjects classified in each class
...
- Global model fit is usually measured with a variety of indices.
...
- Then you can *model* a residual correlation between the *problematic* variables.
  - Change the model one parameter at the time.
  - compare the 2 models.
  - Test for local fit again (not available yet in `latent`).
  - Repeat.
- You should also check how this modification affects the interpretation of the classes (sensitivity analysis).
...
- Do we find profiles of personality characteristics?
...
- What names would you choose for these classes?
...
Sometimes models don't converge or converges to wrong solutions.
Explanations:

(1) Models may be complex and prone to local maxima.

(2) Parameters are estimated too close to their boundaries (i.e., probabilities close to 0 or 1, and variances close to 0).

:::
::: {.column width="30%"}
![](fixing.jpg){width=300%}
:::
:::
...
If any of these happen, you should modify the control arguments accordingly.
...
Sometimes parameters are too close to their boundaries:
```{r, echo=TRUE}
...
Switch to `penalties = TRUE` and check what happens.
...
This shows that penalties are important.
...
After increasing `alpha`, variances become larger.
