...
#'   \code{list(cores = 4L)} requests four OpenMP threads for \code{polyfast()}
#'   polychoric estimation. The two-step ACOV does not use \code{cores} or
#'   explicit OpenMP. NULL uses the moment estimators' own defaults, independently
#'   of \code{control}. Group suffixes and uncertainty
...
lcfa_control <- function(control) {
  ...
  if(control$orth.lambda && !isTRUE(control$std.lv)) {
    stop("orth.lambda = TRUE requires std.lv = TRUE")
  }
  ...
  for(rs in seq_len(control$rstarts)) {
    ...
    for(i in seq_len(dataList$ngroups)) {
      ...
      if(control$orth.lambda) {
        ...
        lambda <- orth(lambda)%*%
          diag(sqrt(colSums(lambda*lambda)), nrow = q)
...
manifolds_lcfa <- function(dataList, data_param, param,
  ...
  for(i in seq_len(dataList$ngroups)) {
    ...
    if(control$orth.lambda) {
      ...
      if(anyNA(psi_numeric) ||
         !isTRUE(all.equal(psi_numeric, diag(q)))) {
        stop("orth.lambda = TRUE requires an identity factor covariance matrix.")
      }
