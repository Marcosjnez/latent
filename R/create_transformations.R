...
create_transforms <- function(transforms_and_labels, param_structures) {
  ...
  # Collect the parameter labels that are not fixed values:
  param_structures_vector <- unname(unique(c(unlist(param_structures))))
...
get_transforms <- function(transforms, structures) {
  ...
  for(i in seq_len(ntransforms)) {
    ...
    for(j in 1:length(labels_in[[i]])) {
      ...
      labels_in_unique <- unname(unlist(structures[labels_in[[i]][j]]))
    ...
    for(j in 1:length(labels_out[[i]])) {
      ...
      labels_out_unique <- unname(unlist(structures[labels_out[[i]][j]]))
