...
getargs_cfa_dwls <- function(estimator, parameter_vector, fixed_vector,
                             corr, W, data, lambda, phi, psi,
  ...
  if(corr == "pearson") {
    ...
    if(estimator == "uls") {
      ...
      if(is.null(W)) {
        asymp <- asymptotic_normal(S)
        W <- matrix(diag(asymp), nrow = p, ncol = p)
        W <- 1 / W
        diag(W) <- 0      }
    }
    ...
    if(nrow(data) == ncol(data)) {
      ...
      if(estimator == "uls") {
        ...
        if(is.null(W)) {
          W <- 1 / DACOV2(nrow(data), S, polychorics$contingency_tables,
  ...
  result <- list(estimator = "cfa_dwls",
                 # data = data,
                 # nfactors = q,
                 # nobs = nobs,
